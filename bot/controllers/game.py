...
from bot.controllers.player import get_player_from_id
from bot.resources.enums import GameStatus
from bot.database.models import Game, Record


async def get_active_games(db_session: AsyncSession) -> list[Game]:
    query = select(Game).filter(Game.status == GameStatus.IN_PROGRESS)
    result = await db_session.execute(query)
    games = result.scalars().all()
    return [game for game in games]


async def get_current_game() -> Game | None:
...
async def get_current_game_stats_for_player(telegram_id: int, game_id: int) -> str:
    record: Record = session.query(Record).filter(Record.game_id == game_id,
                                                  Record.player_telegram_id == telegram_id).scalar()
    ...
    if not buy_out:
        game_stats_for_player = answer['current_game_stats_player_reply'].format(game_id, record.player_id, started,
                                                                                 buy_in, total_pot)
    else:
