...
async def missing_players_input(message: types.Message, state: FSMContext):
    ...
    admin = session.query(User.id).where(User.telegram_id == message.from_user.id)
    ...
    for player in attending_players:
        ...
        session.add(record)
        personal_message = await dp.bot.send_message(chat_id=player.telegram_id, text=caption + text,
                                                     reply_markup=add_keyboard)
...
async def game_finisher(call: types.CallbackQuery):
    ...
    if remaining_players:
        await dp.bot.send_message(chat_id=call.from_user.id,
                                  text=answer['remaining_players_in_game'].format(str(remaining_players)))
    else:
        results = await check_game_balance(game.id)
        if results.delta != 0:
            await dp.bot.send_message(chat_id=call.from_user.id,
                                      text=answer['exit_game_wrong_total_sum'].format(results.delta))
        else:
